...
void makeAllPlots(TString nsel, int applyScaling, int year, int whichCondorJob = 1001, bool doMerging = true){
  ...
  if(nsel.Contains(".root") == true){
    ...
    finalPlot(0,1,"Zep_{VV}","",Form("anaZ/fillhisto_wzAnalysis%d_%d_25.root",whichCondorJob,year),"wzjjsel_zepvv_vbfincl", 0,year,legendBSM.Data(),SF_DY,isBlinded,"",1,applyScaling,mlfitResult.Data(),channelName.Data());
    finalPlot(0,1,"Zep_{VV}","",Form("anaZ/fillhisto_wzAnalysis%d_%d_26.root",whichCondorJob,year),"wzbjjsel_zepvv_vbfincl", 0,year,legendBSM.Data(),SF_DY,isBlinded,"",1,applyScaling,mlfitResult.Data(),channelName.Data());
