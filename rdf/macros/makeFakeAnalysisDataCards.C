...
void makeFakeAnalysisDataCards(TString InputDir = "anaZ", int year = 2022){
  ...
  const int xPtbins = 5;
  const int xEtabins = 5;
  for(unsigned version=0; version<4; version++) {
    for(unsigned ltype=0; ltype<2; ltype++) {
      for(unsigned nPtBin=0; nPtBin<xPtbins; nPtBin++) {
        for(unsigned nEtaBin=0; nEtaBin<xEtabins; nEtaBin++) {
          double yields[3] = {0, 0, 0};
          TH1D *histo_Baseline[nPlotCategories];
          inputFile0 = new TFile(Form("%s/fillhisto_fakeAnalysis1001_%d_%d.root",InputDir.Data(),year,ltype+100+50*version+2*nPtBin+2*xPtbins*nEtaBin), "read");
          ...
          delete inputFile0;
          inputFile1 = new TFile(Form("%s/fillhisto_fakeAnalysis1002_2022_%d.root",InputDir.Data(),ltype+100+50*version+2*nPtBin+2*xPtbins*nEtaBin), "read");
          for(unsigned ic=kPlotData; ic!=nPlotCategories; ic++) {
          ...
          TString outputLimits = Form("output_fake_%d_version%d_l%d_pt%d_eta%d.root",year,version,ltype,nPtBin,nEtaBin);
          outputFile = new TFile(outputLimits, "RECREATE");
          outputFile->cd();
          printf("%d %d %2d %2d\n",version,ltype,nPtBin,nEtaBin);
          for(unsigned ic=kPlotData; ic!=nPlotCategories; ic++) {
	    ...
	    histo_Baseline[ic]->Write();
          ...
          sprintf(outputLimitsCard,"datacard_fake_%d_version%d_l%d_pt%d_eta%d.txt",year,version,ltype,nPtBin,nEtaBin);
          ...
          for (int ic=0; ic<nPlotCategories; ic++){
            if(!histo_Baseline[ic]) continue;
            if(ic == kPlotData || histo_Baseline[ic]->GetSumOfWeights() <= 0) continue;
            if(ic == kPlotNonPrompt) newcardShape << Form("%6.3f ",1.10);
          ...
          for (int ic=0; ic<nPlotCategories; ic++){
            ...
            else                     newcardShape << Form("%6.3f ",1.10);
          }
          newcardShape << Form("\n");
